...
AWS Signature Version 4 request signing, and an S3 client, for
[Milo](https://github.com/milo-language/milo). No SDK, no C dependency — the
whole thing is HMAC-SHA256 over a canonical string, and `std` already has the
primitives.
...
```milo
from "aws" import { Credentials, S3, amzNow }

fn main(): i32 {
    let creds = Credentials.resolve()!          // env, then ~/.aws/credentials
    let s3 = S3.new("https://s3.us-east-1.amazonaws.com", "us-east-1", creds)

    s3.putObject("my-bucket", "notes/hello.txt", "hello from milo\n", "text/plain")!
    print(s3.getObject("my-bucket", "notes/hello.txt")!)

    for m in s3.listObjectsV2("my-bucket", "notes/")! {
        print(m.key + " " + m.size.toString() + " bytes")
    }

    // A URL anyone can GET for five minutes, with no credentials attached.
    print(s3.presignGet("my-bucket", "notes/hello.txt", 300, amzNow()))
    return 0
}
```
...
`tests/sigv4_test.milo and tests/s3_test.milo` also carries the negative controls: one byte of the secret,
